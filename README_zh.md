...
1. 复制 `.env.template` 为 `.env` 并填入你的 API 密钥：
   ```bash
   cp .env.template .env
   ```
...
.
â"â" app/
â"Â â"â" api/                    # FastAPI è·¯ç±ï¼ä¸»è·¯ç± + RAGï¼
â"Â â"â" core/                   # éç½®ãæ°æ®åºãHTTP å®¢æ·ç«¯ãè°åº¦å¨
â"Â â"â" models/                 # SQLModel + Pydantic æ¨¡å
â"Â â"â" scrapers/               # HN / Reddit / GitHub çªè«
â"Â â"â" services/
â"Â â"Â â"â" source_adapters/    # å¯æ'åççæ¿æ¿æ¿çæ¿æ¿
â"Â â"Â â"â" llm/                # LLM è¯åãæè¦ãåå¯¼
â"Â â"Â â"â" rag/                # RAG ç®¡é
â"Â â"Â â"â" repositories/      # æ°æ®åºä»åº
...
â"Â â"â" web/                    # Jinja æ¨¡æ¿ + éæèµæº
â"â" data/
â"Â â"â" chroma/                 # æ¬å°åéåå¨
â"Â â"â" sqlite/                 # æ¬å° SQLite æ°æ®åº
â"â" docs/                      # é¡¹ç®ææ¡£
â"â" logs/                      # è¿è¡æ¥å¿
â"â" scripts/                   # Windows å¯å¨å¨ + Redis å¼å¯¼
â"â" tests/                     # æµè¯å¥ä»¶
â"â" tools/                     # æå å·¥å·ï¼Redis çï¼
```
