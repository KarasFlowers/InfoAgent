...
InfoAgent 是一个基于 FastAPI 的每日科技简报应用。它从多种来源（RSS、Hacker News、Reddit、GitHub 或纯 LLM）聚合内容，使用任意 OpenAI 兼容 LLM 策划结构化摘要，并提供文章级 RAG 对话与反馈驱动的个性化推荐。
...
InfoAgent 通过 [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) 协议暴露核心能力，让 Claude、Cursor、Windsurf 等 AI 助手可以直接查询简报、提问文章、管理偏好。
